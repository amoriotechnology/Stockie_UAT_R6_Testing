...
// Total Employee Salary
    ...
    public function dashboard_query1($invoice_id,$customer_id){
       $sql =  "SELECT (SELECT SUM(total_price) FROM invoice_details a JOIN invoice b ON b.invoice_id = a.invoice_id WHERE a.invoice_id = '" . $invoice_id . "' AND 'create_by' = '".$this->session->userdata('user_id')."' AND b.customer_id = '" . $customer_id . "') as total_amount, 
    (SELECT SUM(paid_amount) FROM invoice_details a JOIN invoice b ON b.invoice_id = a.invoice_id WHERE a.invoice_id = '" . $invoice_id . "' AND 'create_by' = '".$this->session->userdata('user_id')."' AND b.customer_id = '" . $customer_id . "') as total_paid, 
